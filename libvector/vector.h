...
typedef void                (*iterator_t)           (const void *,       void *);
...
VECTOR_EXTERN void *        vector_pop_first        (vector_t);

// Iterator methods
VECTOR_EXTERN void *        vector_begin_iterator   (vector_t);
VECTOR_EXTERN bool          vector_check_iterator   (vector_t,  const void *);
VECTOR_EXTERN void *        vector_next_iterator    (vector_t,  const void *);

#define vector_iterate(_var, _vector) for (_var = vector_begin_iterator(_vector); \
                                           vector_check_iterator(_vector, _var); \
                                           _var = vector_next_iterator(_vector, _var))

#define vector_iterate2(_type, _var, _vector) for (_type _var = vector_begin_iterator(_vector); \
                                                   vector_check_iterator(_vector, _var); \
                                                   _var = vector_next_iterator(_vector, _var))
