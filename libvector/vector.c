...
vector_t vector_init(const void *data, size_t size, size_t count)
{
    ...
    vec = malloc(sizeof(struct __vector_s));
    ...
    memset(vec, 0, sizeof(struct __vector_s));
    if (data)
...
ssize_t vector_indexof(vector_t vec, const void *object, equality_t eq)
{
    ...
    vector_iterate2(void *, obj, vec) // Dogfooding.
    {
        if (eq(obj, object))
            return vector_indexof_ptr(vec, obj);
    }
...
void *vector_begin_iterator(vector_t vec)
{
    assert(vec);
...
bool vector_check_iterator(vector_t vec,  const void *it)
{
    assert(vec);
    
    if (!it)
        return false;
    return ((it - vec->data) / vec->element_size) < vec->count ? true : false;
}

void *vector_next_iterator(vector_t vec,  const void *it)
{
    assert(vec);
    if (((it - vec->data) / vec->element_size) < (vec->count - 1))
        return (void *)it + vec->element_size;
    else
        return NULL;
}
